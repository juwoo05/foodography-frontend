...
async function mockSendCode(email) {
  await new Promise(r => setTimeout(r, 900))
  if (!email.includes('@')) throw new Error('이메일 형식이 올바르지 않습니다.')
  return { success: true, maskedEmail: email.replace(/(.{2})(.*)(@.*)/, (_, a, b, c) => a + '*'.repeat(b.length) + c) }
}
async function mockVerifyCode(email, code) {
  await new Promise(r => setTimeout(r, 700))
  if (code === '123456') return { success: true }
  throw new Error('인증코드가 올바르지 않습니다. (힌트: 123456)')
}
async function mockResetPw() {
...
export default function FindPwPage() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      const res = await mockSendCode(email)
      setMasked(res.maskedEmail); setTimeLeft(CODE_EXPIRE); setStep(1)
  ...
  const handleResend = async () => {
    ...
    try { await mockSendCode(email); setTimeLeft(CODE_EXPIRE); setCode(['','','','','','']); codeRefs.current[0]?.focus() }
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true); setError('')
    try { await mockVerifyCode(email, codeStr); setStep(2) }
    catch (err) { setError(err.message); setCode(['','','','','','']); codeRefs.current[0]?.focus() }
    finally { setLoading(false) }
  }
  ...
  const handleReset = async (e) => {
    ...
    setLoading(true); setError('')
    try { await mockResetPw(); setDone(true) }
    catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }
        ...
        <div className={styles.card}>
            ...
            <div className={styles.stepIndicator}>
              {STEPS.map((label, idx) => (
                <React.Fragment key={label}>
                  <div className={`${styles.stepBubble} ${idx < step ? styles.stepBubbleDone : idx === step ? styles.stepBubbleActive : ''}`}>
                    {idx < step ? <CheckCircle size={13} /> : idx + 1}
                  </div>
                  {idx < STEPS.length - 1 && <div className={`${styles.stepLine} ${idx < step ? styles.stepLineDone : ''}`} />}
                </React.Fragment>
              ))}
            </div>
          )}
          ...
          /* step 0 */
            ...
            <form className={styles.form} onSubmit={handleSend} noValidate>
              ...
              <button type="submit" className={styles.submitBtn} disabled={loading}>
                {loading ? <span className={styles.spinner} /> : <><span>인증코드 전송</span><ArrowRight size={16} /></>}
              </button>
            </form>
            ...
            <form className={styles.form} onSubmit={handleVerify} noValidate>
              ...
              <div className={styles.fieldWrap}>
                <label className={styles.label}>
                  인증코드 6자리
                  {timeLeft > 0 && (
                    <span style={{ color: timeLeft < 60 ? '#F85149' : '#2ECC71', fontFamily: 'Space Mono,monospace', marginLeft: 8 }}>
                      {mm}:{ss}
                    </span>
                  )}
                </label>
                  ...
                  {code.map((digit, idx) => (
                    <input key={idx} ref={el => (codeRefs.current[idx] = el)}
                      ...
                      style={{
                        ...
                        borderRadius: 10, color: '#E6EDF3', outline: 'none', transition: 'border-color .15s',
                      }}
                    />
                  ))}
                </div>
                {error && <p className={styles.fieldErr} style={{ justifyContent: 'center' }}><AlertCircle size={11} />{error}</p>}
              </div>
              <button type="submit" className={styles.submitBtn} disabled={loading || timeLeft === 0}>
                {loading ? <span className={styles.spinner} /> : <><span>인증 확인</span><ArrowRight size={16} /></>}
              </button>
              <button type="button" className={styles.ghostBtn} onClick={handleResend} disabled={resending || timeLeft > 150}>
            ...
            <form className={styles.form} onSubmit={handleReset} noValidate>
              <div className={styles.fieldWrap}>
                ...
                <div className={styles.inputWrap}>
                  ...
                  <input type={showPw ? 'text' : 'password'} className={styles.input} placeholder="새 비밀번호 입력"
                ...
                {password.length > 0 && (
                  <div className={styles.pwRules}>
                    {PW_RULES.map(r => (
                      <span key={r.label} className={`${styles.pwRule} ${r.test(password) ? styles.pwRuleOk : ''}`}>✓ {r.label}</span>
                    ))}
                  </div>
                )}
              </div>
              <div className={styles.fieldWrap}>
                ...
                <div className={`${styles.inputWrap} ${confirm && !confirmOk ? styles.inputError : confirm && confirmOk ? styles.inputOk : ''}`}>
                  <Lock size={16} className={styles.inputIcon} />
                  <input type={showPw ? 'text' : 'password'} className={styles.input} placeholder="비밀번호 재입력"
                    value={confirm} onChange={e => setConfirm(e.target.value)} />
                  {confirm && confirmOk && <CheckCircle size={15} style={{ color: '#2ECC71', flexShrink: 0 }} />}
                </div>
              </div>
              {error && <div className={styles.serverErr}><AlertCircle size={14} />{error}</div>}
              <button type="submit" className={styles.submitBtn} disabled={loading}>
                {loading ? <span className={styles.spinner} /> : <><span>비밀번호 변경 완료</span><ArrowRight size={16} /></>}
              </button>
            </form>
          )}
          ...
          <div className={styles.bottomLinks}>
            <div className={styles.bottomLinkRow}>
              <Link to="/login" className={styles.linkBtn}><ChevronLeft size={12} style={{ verticalAlign: 'middle' }} />로그인</Link>
